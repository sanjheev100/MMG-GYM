...
const PaidCustomers = () => {
  ...
  const [paidDownload, setPaidDownload] = useState([])
  ...
  const loadPaidMembers = () => {
    ...
    getPaidMembers(user.token, payload)
      .then((res) => {
        setMembers(res.data)
      })
      .catch((err) => {
  ...
  const loadTotalPaidcount = () => {
    getPaidMembersCount(user.token)
      .then((res) => {
  ...
  const csvReport = {
    filename: `PaidCustomers_${moment(today).format('DD-MMM-YYYY')}.csv`,
    headers: headers,
    data: paidDownload,
  }
  const handlePaidCustomerDownload = () => {
    setButtonLoading(true)
    getAllPaidMembers(user.token).then((res) => {
      if (res.data.membersCheck) {
        setPaidDownload(res.data.members)
  ...
  return (
    <>
      <Container>
        <h4 className='text-center'>Paid Customers List</h4>
        <div>
          <form
                ...
                <>
                  {showButton ? (
                    ...
                    <Button
                      className='btn-raised mt-3'
                      variant='primary'
                      onClick={() => handlePaidCustomerDownload()}
                      style={{ top: '-8px', height: '40px' }}
                    >
                      Get File
                    </Button>
              ...
              <tbody>
                {members.map((m) => (
                  <MemberTable
                    member={m}
                    key={m._id}
                    loadAllCustomers={loadPaidMembers}
                  />
                ))}
              </tbody>
            </table>
